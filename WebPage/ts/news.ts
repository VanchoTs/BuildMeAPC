document.addEventListener('DOMContentLoaded', async () => {
    ...
    const newsForm = document.getElementById('create-news-form') as HTMLFormElement;
    ...
    if (toggleBtn && newsForm && cancelBtn) {
        ...
        newsForm.addEventListener('submit', async (e) => {
            ...
            const titleInput = document.getElementById('news-title') as HTMLInputElement;
            const imageInput = document.getElementById('news-image') as HTMLInputElement;
            const catchPhraseInput = document.getElementById('news-catch-phrase') as HTMLInputElement;
            const contentInput = document.getElementById('news-content') as HTMLTextAreaElement;
    ...
    function renderNewsGrid(news: any[]) {
        if (!newsContainer) return;
        newsContainer.innerHTML = '';

        if (news.length === 0) {
            newsContainer.innerHTML = '<p>No news articles published yet.</p>';
            return;
        }
    ...
    function showSingleArticle(article: any) {
