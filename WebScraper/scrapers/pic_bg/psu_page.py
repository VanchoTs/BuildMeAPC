...
from ai.psu_brands import PSU_BRAND_MAP, PSU_BRAND_TOKENS
...
def _normalize_brand_token(token: str) -> str:
    ...
    """
    return PSU_BRAND_MAP.get(token.upper(), token.title())


def _parse_brand_model(title: str, prioritized_brand: str | None = None):
    """
    Heuristically extracts PSU brand and model from the title, stripping common PSU-related keywords.
    ...
    if prioritized_brand:
        ...
        # Remove any matching known brand token from title to get model
        for token in PSU_BRAND_TOKENS:
    ...
    s = re.sub(r"(?i)^(?:PSU|POWER SUPPLY|ЗАХРАНВАНЕ)\s*", "", s).strip()
...
def _normalize_breadcrumb_brand_candidate(value: str | None) -> Optional[str]:
    """
    Validates if a string candidate from breadcrumbs matches a known PSU brand.
    """
    if not value:
        return None
...
def _extract_breadcrumb_brand(soup: BeautifulSoup) -> Optional[str]:
    ...
    # and can lead to misidentifying the brand as whatever is first in the sidebar list (e.g. ASUS).
...
def parse_psu_page(html: str, url: str) -> dict:
    """
    Main entry point for parsing a PSU product page.
    """
