        ...
        for url in sorted(all_urls):
            ...
            try:
                ...
                def _infer_model_from_text(text: str):
                    ...
                    m = re.search(
                        r"ryzen\s+([3579])\s*-?\s*(\d{4,5}[a-z]{0,2}|\d{4,5}[a-z]\d[a-z]|\d{4,5}x3d)",
                        lowered,
                        flags=re.IGNORECASE,
                    )
                    if m:
                ...
                def _model_has_packaging(m: str) -> bool:
                    ...
                    upper = str(m).upper()
                    if any(
                        k in upper for k in ("MPK", "WOF", "BOX", "TRAY", "SBX", "KIT")
                    ):
                        return True
                ...
                def _model_is_placeholder(m: str) -> bool:
                    ...
                    upper = str(m).upper()
                    if any(
                        k in upper for k in ("64-BIT", "64 BIT", "PROCESSOR", "CPU")
                    ) and not re.search(r"\d{4,5}", upper):
                        return True
                ...
                def _normalize_model_brand(
                    model: str | None, brand: str | None, text: str
                ):
                    if not model:
                        return None, brand
                    m = str(model).strip()
                    ...
                    if re.match(
                        r"^I[3579][-\s]?\d{4,5}[A-Z]{0,3}$", m, flags=re.IGNORECASE
                    ):
                        m = "Core " + m
                ...
                final_model, final_brand = _normalize_model_brand(
                    final_model, final_brand, text_source
                )
                ...
                if final.get("cores") is None:
                    spec_cores = _spec_value(["cores", "ядр"])
                    if spec_cores:
                ...
                if final.get("threads") is None:
                    spec_threads = _spec_value(["threads", "ниш", "thread"])
                    if spec_threads:
                    ...
                    if final.get("threads") is None:
                        threads = _try_extract(
                            [r"(\d+)\s*(?:threads|thread|нишки|тред)"]
                        )
                        if threads:
                            try:
                                final["threads"] = int(threads)
                ...
                if final.get("tdp") is None:
                    spec_tdp = _spec_value(["tdp", "power", "мощност", "топлина"])
                    if spec_tdp:
                ...
                if final.get("base_clock") is None:
                    spec_base = _spec_value(
                        ["base", "основ", "clock", "frequency", "честота"]
                    )
                    if spec_base:
                ...
                if final.get("boost_clock") is None:
                    spec_boost = _spec_value(["boost", "turbo", "max"])
                    if spec_boost:
                ...
                if final.get("socket") is None:
                    spec_socket = _spec_value(["socket", "сокет"])
                    if spec_socket:
                ...
                if final.get("memory_type") is None:
                    spec_mem = _spec_value(["memory", "ram", "памет"])
                    if spec_mem:
