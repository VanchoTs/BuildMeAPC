...
from ai.prompts import PSU_PROMPT
...
def parse_psu(html: str, name: str, price: float, url: str) -> dict:
    """
    Parses PSU specifications from HTML content using an LLM.
    
    This function:
    1. Prepares the content by extracting relevant spec text.
    2. Sends the text to the LLM with a PSU extraction prompt.
    3. Handles and repairs JSON output from the AI.
    4. Normalizes wattage, efficiency, and modularity data.
    """
    ...
    raw_response = run_llm_completion(
        llm,
        lambda content_text: PSU_PROMPT.format(
    ...
    return {
        ...
        "power_w": _to_int(data.get("power_w")),
